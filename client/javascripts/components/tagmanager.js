  ...
  publicMethods = {
    pushTag: function (tag, ignoreEvents, externalTagId) {
      ...
      if (alreadyInList) {
        ...
        html += '<i class="zmdi zmdi-close zmdi-hc-fw"></i></a></span>';
        $el = $(html);


        var typeAheadMess = $self.parents('.tag-manage')[0] !== undefined;
        $self.parents('.tag-manage').find('.skill-list').append($el);
